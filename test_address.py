...
class TestAddress(unittest.TestCase):
	def test_generate(self):
		...
		diversifier = hash_to_scalar('div',spend.s1,lookahead)
		...
		self.assertEqual(public.Q0,(diversifier + spend.s1)*params.F)
		self.assertEqual(public.Q1,spend.s1*public.Q0)
		self.assertEqual(public.Q2,(diversifier + spend.s2)*params.F + spend.r*params.G)

		for i in range(lookahead+2):
			entry = (hash_to_scalar('div',spend.s1,i) + spend.s2)*params.F + spend.r*params.G
			if i > lookahead:
